...
Color3 eval_lambertian(Vector3 view, Vector3 normal, Vector3 light, PathTracedMaterial material) {
    ...
    return vec3_scale(material.base_color, n_dot_l * M_1_PI);
}

Vector3 sample_lambertian(Vector3 surface_normal, PathTracedMaterial material_unused){
    return vec3_normalized(vec3_add(surface_normal, random_point_in_sphere(1)));
}
...
Vector3 eval_indirect_lambertian(Color3* sample_weight, Vector3 surface_normal, PathTracedMaterial material){
    Vector3 sample_dir = sample_lambertian(surface_normal, material);
    *sample_weight = material.base_color;
    return sample_dir;
}
